...
do
    ...
    cp bin/start.sh $dir
    echo -e "#!/bin/bash\n\nexport HADOOP_HOME=/cluster/software/hadoop\nexport HADOOP_USER_NAME=work\nbash start.sh camus_${topic}.properties /camus/lock/${topic}" > "$dir/start_${topic}.sh" 
    echo -e "#!/bin/bash\n\nexport HADOOP_HOME=/cluster/software/hadoop\nexport HADOOP_USER_NAME=work\nbash start.sh camus_${topic}_init.properties /camus/lock/${topic}" > "$dir/start_init_${topic}.sh" 
